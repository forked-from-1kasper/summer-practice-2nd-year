...
Всюду будем использовать стандартные логические обозначения ($\forall$, $\exists$, $\land$, $\lor$, $\Rightarrow$) и,
также стандартно, $f \circ g$ для композиции функций (то есть для функции $x \mapsto f(g(x))$), $\card(X)$ для мощности множества.
...
\begin{theorem*}
  Если $f \circ (n \mapsto n + k)$ сходится, где $k \in \N$, то $f$ сходится к тому же числу.
\end{theorem*}
