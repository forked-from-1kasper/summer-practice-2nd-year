...
\end{theorem*}
Каждому $A$ сопоставим некоторое счётное плотное подмножество и обозначим его $\D(A)$.
...
\begin{proof}
Из ??? следует $ \Lim(\omega(a_0, a_1, ..., a_m)) \subseteq \overline{\{ a_0, a_1, ..., a_m \}} = \{ a_0, a_1, ..., a_m \} $.

Докажем $ \{ a_0, a_1, ..., a_m \} \subseteq \Lim(\omega(a_0, a_1, ..., a_m)) $.
Возьмём некоторое $a_i$, тогда
\begin{equation*}
  \underset{n \rightarrow + \infty}{\lim} \omega(n(m + 1) + i) = \underset{n \rightarrow + \infty}{\lim} a_i = a_i
\end{equation*}
...
\begin{proof}
Поскольку $\kappa_A(\phi(n)) \in A$, из ??? получаем, что $\Lim(\kappa_A \circ \phi) \subseteq \overline{A}$.

Обратно, пусть $r \in \overline{A}$. Обозначим $k = \kappa_A^{-1}(r)$. Тогда:
\begin{align*}
...
Обозначим $k_n = \kappa_A^{-1}(g(n))$ и $k^\prime_n = k_0 + k_1 + ... + k_n$. Аналогично получаем:
\begin{align*}
   \ & \kappa_A \circ \phi \circ (n \mapsto k_n + k^\prime_n(k^\prime_n + 1)/2) \\
  =\ & (n \mapsto \kappa_A(\phi(k_n + k^\prime_n(k^\prime_n + 1)/2))) \\
  =\ & (n \mapsto \kappa_A(k_n)) \\
  =\ & (n \mapsto \kappa_A(\kappa_A^{-1}(g(n)))) \\
  =\ & (n \mapsto g(n)) \\
  =\ & g
\end{align*}
...
\begin{proof}
Следует из теорем ???, ???, ??? и того, что $\D(A)$ плотно в $A$.
\end{proof}
