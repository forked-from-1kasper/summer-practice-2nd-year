...
Рассмотрим проколотую окрестность $(r - q_0; r + q_0) \cap [x_i, y_i] \setminus \{ r \}$. Снова в силу того, что $a_i$ — биекция, в этой окрестности
...
Далее рассмотрим окрестность $(r - q_0/2; r + q_0) \cap [x_i, y_i] \setminus \{ r \}$. Исключим из неё все рациональные точки $q$ такие,
что $a_i^{-1}(q) \leq \mathrm{max}\{k_0, k_1\}$ (то есть, в частности, и само $q_1$).

Аналогичным образом выбирая далее точки в окрестностях $(r - q_0/n; r + q_0/n) \cap [x_i, y_i] \setminus \{ r \}$,
...
Обратно, пусть $r$ — предельная точка $a_i$. Следовательно, существует подпоследовательность $a_i \circ f$ такая,
что $\underset{n \rightarrow + \infty}{\lim} a_i(f(n)) = r$. Однако, по заданию, $x_i \leq a_i(f(n)) \leq y_i$.
Переходя к пределу, получим, что $x_i \leq r \leq y_i$, то есть $r \in [x_i, y_i]$.
