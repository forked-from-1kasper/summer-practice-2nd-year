...
ссылаясь на утверждения о том, что такое поле единственно с точностью до изоморфизма и существует \cite{Kudr1}.
...
Точку $x \in \R$ назовём {\bf предельной точкой} множества $A \subseteq \R$, если в любой окрестности этой точки
...
\begin{equation*}
  \forall \varepsilon > 0: \exists N \in \N: \forall n \geq N : |f(n) - A| < \varepsilon
\end{equation*}
...
Приведём без доказательства несколько известных утверждений \cite{Kudr1, Tsikh}.
...
\begin{proof}
  Пусть выполнено $\forall \varepsilon > 0: \exists N_\varepsilon \in \N: \forall n \geq N_\varepsilon : |f(n) - A| < \varepsilon$.
  ...
  Тогда для всех $n \geq M_\varepsilon$ выполняется $g(n) \geq g(M_\varepsilon) \geq N_\varepsilon$, следовательно $|f(g(n)) - A| < \varepsilon$.
\end{proof}

\begin{theorem}
  \label{two-policemans}
  Пусть даны две сходящиеяся последовательности $f$ и $g$ такие, что $\limit f(n) = \limit g(n) = A$,
  а также последовательность $h : \N \rightarrow \R$.

  Если для всех $n$ верно, что $f(n) \leq h(n) \leq g(n)$, то $\limit h(n) = A$.
\end{theorem}

\begin{proof}
  Зафиксируем некоторое $\varepsilon > 0$.
  Тогда $\exists N_\varepsilon \in \N: \forall n \geq N_\varepsilon : |f(n) - A| < \varepsilon$
  и $\exists M_\varepsilon \in \N: \forall n \geq M_\varepsilon : |g(n) - A| < \varepsilon$.

  Обозначим $K_\varepsilon = \mathrm{max} \{ N_\varepsilon, M_\varepsilon \}$ и возьмём некоторое $n \geq K_\varepsilon$.
  По заданию $K_\varepsilon$, $n \geq K_\varepsilon \geq N_\varepsilon$ и $n \geq K_\varepsilon \geq M_\varepsilon$.

  Известно, что $|h(n) - A| < \varepsilon \Leftrightarrow A - \varepsilon < h(n) < A + \varepsilon$.
  Поскольку $n \geq N_\varepsilon$ и $n \geq M_\varepsilon$, также верно, что
  $A - \varepsilon < f(n) < A + \varepsilon$ и $A - \varepsilon < g(n) < A + \varepsilon$.

  Объединяя это с условием $f(n) \leq h(n) \leq g(n)$,
  получим $A - \varepsilon < f(n) \leq h(n) \leq g(n) < A + \varepsilon$.
\end{proof}

\begin{lemma}
  \label{lim-subset}
  Для всяких $A, B \subseteq \R$ верно $A \subseteq B \Rightarrow \Lim(A) \subseteq \Lim(B)$.
\end{lemma}

\begin{proof}
  Вытекает напрямую из определения.
\end{proof}

\begin{theorem*}
  Во всяком замкнутом $A \subseteq \R$ можно выделить не более чем счётное плотное подмножество.
\end{theorem*}
...
\begin{proof}
  ...
  Докажем сначала, что $\Lim(\D(A)) \subseteq A$. Действительно, поскольку $\D(A) \subseteq A$,
  то из леммы \ref{lim-subset} и того, что $A$ замкнуто, получаем $\Lim(\D(A)) \subseteq \Lim(A) \subseteq A$.

  Теперь докажем, что $A \subseteq \overline{\D(A)}$. Пусть $c \in A$. Если $c \in \D(A)$, то утверждение, очевидно, выполнено;
  ...
  В силу аксиомы Архимеда существует $r \in \Q$ такое, что $\varepsilon/3 < r < 2\varepsilon/3$.
...
\begin{proof}
  Докажем, что $\Lim(A) = \varnothing$. Пусть $r \in \Lim(A)$ и $A = \{ a_0, a_1, ..., a_m \}$.

  Возьмём $E = \mathrm{min} \{ |r - a_i|\ |\ 0 \leq i \leq m \}$ и $\varepsilon = E/2$.
  Тогда, по определению, существует некоторый $a_i$ такой, что $|r - a_i| < \varepsilon = E/2$ и $r \neq a_i$.
  Однако из задания $E$ следует, что $E \leq |r - a_i| < E/2$, — противоречие.

  Следовательно, $\Lim(A) \subseteq \varnothing$, но $\varnothing \subseteq \Lim(A)$ очевидно; поэтому $\Lim(A) = \varnothing$.

  Откуда получаем, что $\overline{A} = A \cup \Lim(A) = A \cup \varnothing = A$.
\end{proof}
...
\begin{proof}
  Действительно, если $\D(A)$ конечно, то в силу леммы \ref{clos-of-finite-set} имеем $\overline{\D(A)} = \D(A)$,
...
\begin{proof}
  Возьмём $r \in \Lim(f)$. По определению, существует подпоследовательность $f \circ g$ такая, что $\limit f(g(n)) = r$.

  По определению предела, $\forall \varepsilon > 0: \exists N \in \N: \forall n \geq N : |f(g(n)) - r| < \varepsilon$.
  Зафиксируем $\varepsilon > 0$ и возьмём $n$ такое, что $f(g(n)) \neq r$.

  Тогда $|r - f(g(n))| < \varepsilon$, $f(g(n)) \in A$ и $f(g(n)) \neq r$, но это означает, что $r$ — предельная точка в $A$.
  Следовательно, $r \in \Lim(A) \in \overline{A}$.
\end{proof}
...
\begin{proof}
  Пусть $f : \N \rightarrow \R$. Докажем, что $\Lim(\Lim(f)) \subseteq \Lim(f)$.

  Действительно, пусть $r \in \Lim(\Lim(f))$. Рассмотрим окрестность $(r - 1; r + 1)$.
  По определению предельной точки, в этой окрестности существует некоторое $r_0 \in \Lim(f)$ такое, что $r \neq r_0$.
  По определению предельной точки последовательности, существует некоторая подпоследовательность $f \circ g_0$ такая,
  что $\limit f(g_0(n)) = r_0$.

  Зафиксируем $\varepsilon = |r - r_0|$, выберем некоторый номер $n_0 \geq N_\varepsilon$
  и обозначим $k_0 = g_0(n_0)$. По определению предела последовательности, $|f(k_0) - r_0| < \varepsilon = |r - r_0|$.

  Откуда $|f(k_0) - r| = |f(k_0) - r_0 + r_0 - r| \leq |f(k_0) - r_0| + |r_0 - r| < 2|r - r_0| = 2$.

  Далее, рассмотрим окрестность $(r - 1/2; r + 1/2)$. Аналогично, возьмём $r_1 \in \Lim(f)$ в этой окрестности
  и $f \circ g_1$ такое, что $\limit f(g_1(n)) = r_1$.

  Зафиксируем $\varepsilon = |r - r_0|$, выберем некоторый номер $n_1 \geq N_\varepsilon$ такой,
  что $k_1 = g_1(n_1) > k_0$ (он существует в силу того, что $g_1$ строго возрастает).

  Аналогично получаем, что $f(k_1) - r < 2|r - r_1| = 1$.

  Продолжая, получим строго возрастающую последовательность $(n \mapsto k_n)$ такую, что $|f(k_n) - r| < 2/(n + 1)$,
  то есть, $r - 2/(n + 1) < f(k_n) < r + 2/(n + 1)$.

  Поскольку $\limit (r - 2/(n+1)) = \limit (r + 2/(n + 1)) = r$, то, по теореме \ref{two-policemans},
  $\limit f(k_n) = r$, но это и означает, что $r \in \Lim(f)$.
\end{proof}
...
\section{Вспомогательные конструкции}
...
\begin{proof}
  Пусть $r \in \Lim(\id)$, то есть существует подпоследовательность $\id \circ f$
  такая, что $\limit \id(f(n)) = \limit f(n) = r$, где $f$ строго возрастает.

  Зафиксируем $\varepsilon = 1/2$. Тогда $\exists N \in \N: \forall n \geq N: |f(n) - r| < 1/2$.
  В частности, $r - 1/2 < f(N)$ и $f(N + 1) < r + 1/2$. Поскольку $f$ строго возрастает, верно, что $f(N) < f(N + 1)$.

  $f(N), f(N + 1) \in \N$, поэтому $f(N + 1) - f(N) \geq 1$.
  Однако тогда $1 \leq f(N + 1) - f(N) < r + 1/2 - r + 1/2 = 1$ — противоречие.
\end{proof}
...
\begin{proof}
Из леммы \ref{clos-of-finite-set} и теоремы \ref{lim-f-subset-clos-A} следует, что
\begin{equation*}
...
Пусть теперь $k_n = 0$ лишь для конечного числа номеров. Возьмём наибольший такой номер $k$ и обозначим $g^\prime = g \circ (n \mapsto n + k + 1)$.
...
\bibitem{Kudr1} Кудрявцев Л. Д. Курс математического анализа. В 3-х томах.
Том 1. Дифференциальное и интегральное исчисления функций одной переменной. — Дрофа, 2003. — 703 с. — ISBN 5-7107-4119-1, 5-7107-5004-2.

\bibitem{Kudr2} Кудрявцев Л. Д. Курс математического анализа. В 3-х томах.
Том 2. Ряды. Дифференциальное и интегральное исчисления функций многих переменных. — Дрофа, 2004. — 720 с. — ISBN 5-7107-5003-4, 5-7107-5004-2.

\bibitem{Tsikh} Введение в специальность «Математика»: Учеб. пособие / А. К. Цих; Краснояр. гос. ун-т. — Красноярск: КГУ, 1997. — 159 с. — ISBN 5-7638-0057-5.

\bibitem{counterexamples-in-analysis} Б. Гелбаум, Дж. Олмстед. Контрпримеры в анализе / Пер. с англ. Б. И. Голубова. —
Москва: Издательство ЛКИ, 2010. — 248 с. — ISBN 978-5-382-01039-7.

\bibitem{math-se} Mathematics Stack Exchange [Электронный ресурс] / Prove that a subset of a separable set is itself separable. —
Режим доступа: \url{https://math.stackexchange.com/questions/516886/prove-that-a-subset-of-a-separable-set-is-itself-separable}. — Яз. англ.
